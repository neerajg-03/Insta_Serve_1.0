...
const AdminDashboard: React.FC = () => {
  ...
  const handleViewUserDocuments = async (user: any) => {
    try {
      // Only show documents for providers
      if (user.role !== 'provider') {
        toast.error('Documents are only available for providers');
        return;
      }

      // Fetch user details with KYC documents
      const response = await adminAPI.getUser(user._id);
      const userWithDocs = response.user;

      if (userWithDocs.kycDocuments && userWithDocs.kycDocuments.length > 0) {
        // Show the first document by default, or create a special view for all documents
        setSelectedDocument({
          ...userWithDocs.kycDocuments[0],
          userName: userWithDocs.name,
          userEmail: userWithDocs.email,
          allDocuments: userWithDocs.kycDocuments
        });
        setShowDocumentViewer(true);
      } else {
        toast.error('No KYC documents found for this provider');
      }
    } catch (error: any) {
                      ...
                      <div className="flex justify-between items-start">
                        ...
                        <div className="flex items-center space-x-2 ml-4">
                          <button 
                            ...
                            title={user.role === 'provider' ? 'View KYC Documents' : 'Documents only available for providers'}
                          >
          ...
          <div className="relative top-10 mx-auto p-5 border w-11/12 max-w-4xl shadow-lg rounded-md bg-white">
            <div className="mt-3">
              ...
              <div className="space-y-4">
                  ...
                  <div className="bg-gray-50 rounded-lg p-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                      <div>
                        ...
                        <p className="text-gray-900 mt-1">{selectedDocument.userName}</p>
                      </div>
                      <div>
                        <span className="font-medium text-gray-600">Email:</span>
                        <p className="text-gray-900 mt-1">{selectedDocument.userEmail}</p>
                      </div>
                    </div>
                  </div>
                )}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                    ...
                    <div>
                      ...
                      <p className="text-gray-900 mt-1">
                        {selectedDocument.documentNumber || 'Not provided'}
                      </p>
                    </div>
                    <div>
                ...
                {/* All Documents (if viewing from Users section) */}
                {selectedDocument.allDocuments && selectedDocument.allDocuments.length > 1 && (
                  <div className="bg-gray-50 rounded-lg p-4">
                    <h4 className="font-semibold text-gray-900 mb-3">All KYC Documents</h4>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      {selectedDocument.allDocuments.map((doc: any, index: number) => (
                        <div 
                          key={index}
                          className={`p-3 bg-white rounded-lg border-2 cursor-pointer transition-colors ${
                            doc.documentUrl === selectedDocument.documentUrl 
                          ...
                          onClick={() => setSelectedDocument({
                            ...
                            allDocuments: selectedDocument.allDocuments
                          })}
                        >
                          <div className="flex items-center justify-between">
                            <div className="flex items-center space-x-3">
                ...
                <div className="bg-gray-50 rounded-lg p-4">
                  ...
                  {selectedDocument.documentUrl ? (
                    <div className="space-y-3">
                      ...
                      {(selectedDocument.documentUrl.toLowerCase().includes('.jpg') || 
                        selectedDocument.documentUrl.toLowerCase().includes('.jpeg') || 
                        selectedDocument.documentUrl.toLowerCase().includes('.png') || 
                        selectedDocument.documentUrl.toLowerCase().includes('.gif')) ? (
                        <div className="bg-white rounded-lg border border-gray-200 p-4">
                          <img 
                            ...
                            onError={(e) => {
                              e.currentTarget.src = 'data:image/svg+xml;base64,';
                            }}
                          />
                        </div>
                      ) : (
                      ...
                      <div className="bg-white rounded-lg border border-gray-200 p-3">
                        <p className="text-xs font-medium text-gray-600 mb-1">Document URL:</p>
                        <p className="text-xs text-gray-500 break-all">{selectedDocument.documentUrl}</p>
                      </div>
                    </div>
                  ) : (
                    <div className="text-center py-8">
                      <DocumentTextIcon className="w-16 h-16 text-gray-300 mx-auto mb-4" />
                      <p className="text-gray-600">No document file available</p>
                    </div>
                  )}
                </div>
              </div>
