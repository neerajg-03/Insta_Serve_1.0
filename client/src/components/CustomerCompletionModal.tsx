...
const CustomerCompletionModal: React.FC<CustomerCompletionModalProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setLoading(true);
      const response = await bookingsAPI.verifyCompletionCode(booking._id, { completionCode: inputCode });
      ...
      <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full mx-4 transform transition-all">
        ...
        <div className="p-6">
          ...
          <div className="mb-6 p-4 bg-blue-50 rounded-lg border border-blue-200">
            ...
            <p className="text-sm text-blue-700">{booking.service?.title || 'Service'}</p>
            <p className="text-xs text-blue-600 mt-1">Provider: {booking.provider?.name || 'Provider'}</p>
          </div>
