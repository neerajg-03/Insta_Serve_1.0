...
const Home: React.FC = () => {
  ...
  useEffect(() => {
    const fetchReviews = async () => {
      try {
        setLoading(true);
        // Use direct axios call for public endpoint to avoid auth issues
        const API_BASE_URL = process.env.REACT_APP_API_URL || 'http://localhost:5000/api';
        const response = await axios.get(`${API_BASE_URL}/bookings/reviews?limit=6`);
        
        if (response.data.success) {
          setTestimonials(response.data.testimonials);
          console.log('Reviews loaded successfully:', response.data.testimonials.length);
        } else {
  ...
  const fallbackTestimonials: Testimonial[] = [
    {
      ...
      comment: 'InstaServe made it so easy to find a reliable cleaner. The provider was professional and did an amazing job!',
      rating: 5
    },
    {
      _id: 'fallback-2',
      ...
      comment: 'Quick response time and expert service. My refrigerator was fixed within hours. Highly recommend!',
      rating: 5
    },
    {
      _id: 'fallback-3',
      ...
      comment: 'Found an amazing spa service through InstaServe. The booking process was seamless and the experience was wonderful.',
      rating: 5
    }
  ];
  ...
  // Debug logging
  useEffect(() => {
    console.log('Testimonials state:', { 
      testimonials: testimonials.length, 
      loading, 
      error,
      displayTestimonials: displayTestimonials.length 
    });
  }, [testimonials, loading, error, displayTestimonials]);
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-32">
          <div className="text-center">
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center mb-12">
              ...
              <button
                onClick={() => {
                  console.log('Become a Provider button clicked');
                  navigate('/register');
                }}
            ...
            {categories.map((category, index) => {
              ...
              return (
                <button
                  ...
                  className="group relative bg-white rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-2 overflow-hidden border border-gray-100 w-full text-left"
                >
