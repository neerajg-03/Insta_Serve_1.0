// Navigation Modal Component with Turn-by-Turn Directions using Google Maps
...
const NavigationModal: React.FC<NavigationModalProps> = ({
  ...
  useEffect(() => {
    if (!isOpen || !mapRef.current || !googleMapsLoaded || mapInstanceRef.current) return;
  ...
  const fetchRouteData = async () => {
    ...
    try {
      ...
      // Check if Maps API is properly initialized (billing enabled)
      ...
      directionsService.route(request, (result: any, status: string) => {
        if (status === 'OK' && result) {
          console.log('Google Maps Directions API response:', result);
          ...
          const processedRoute: RouteData = {
            ...
            geometry: [], // Google Maps handles geometry via DirectionsRenderer
  ...
  const updateMapWithRoute = (directionsResult: any, provLocation: { lat: number; lng: number }, custLocation: { lat: number; lng: number }) => {
    ...
    bounds.extend(new window.google.maps.LatLng(provLocation.lat, provLocation.lng));
    bounds.extend(new window.google.maps.LatLng(custLocation.lat, custLocation.lng));
